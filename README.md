...
This stage contains the HTTP foundation, environment configuration, PostgreSQL connectivity, health endpoints, a migration runner, authentication foundation, administrative CRUDs, event ingestion, dispatcher scheduling, observability, and the first integration source/destination foundation. It still does not include complete business modules, final roles and permissions, Redis, external queues, real n8n HTTP delivery, or complete multi-tenant behavior beyond the implemented tenant isolation rules.
...
## Integration Destinations Foundation

Sprint 6.6A separates event entry points from external processing targets.

- `IntegrationSource` represents where an event enters the platform.
- `IntegrationDestination` represents where that event may be processed later.

Migration `006_create_integration_destinations.sql` adds `integration_destinations` with immutable public codes in the format `DST000001`, generated by `integration_destinations_code_seq`. The table stores `tenant_id`, `name`, `slug`, `type`, `status`, non-sensitive `config`, and timestamps. For this sprint the only accepted destination `type` is `n8n`, and status is `active` or `inactive`.
...
`config` is intentionally limited to non-sensitive operational data and defaults to `{}`. This sprint does not store webhook URLs, tokens, passwords, API keys, bearer tokens, secrets, credentials, webhook secrets, or any transport credential.

`EventProcessorResolver` prepares the future processing path:

```text
event -> source -> destination -> resolver -> processor
```

For `type = n8n`, the resolver returns `N8nEventProcessor`. The n8n processor is a placeholder only: if invoked before Sprint 6.6B, it throws a permanent sanitized failure, `N8n destination transport is not configured.` It performs no HTTP, cURL, webhook call, credential lookup, retry decision, database access, or event mutation. The existing dispatcher still uses `SimulatedEventProcessor`, preserving current retry, scheduler, stale recovery, observability, and webhook behavior.
