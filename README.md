...
This stage contains the HTTP foundation, environment configuration, PostgreSQL connectivity, health endpoints, a migration runner, authentication foundation, administrative CRUDs, event ingestion, dispatcher scheduling, observability, integration sources/destinations, and real n8n HTTP delivery. It still does not include complete business modules, final roles and permissions, Redis, external queues, credential vaults, or complete multi-tenant behavior beyond the implemented tenant isolation rules.
...
app/
  ...
  Services/
    ...
    IntegrationDestinationService.php
    EventProcessorResolver.php
    N8nEventProcessor.php
  Repositories/
    ...
    IntegrationDestinationRepository.php
    IntegrationDestinationRepositoryContract.php
...
database/
  migrations/
    ...
    006_create_integration_destinations.sql
  seeds/
docs/
public/
  index.php
...
Sprint 6.6B turns the n8n processor into real HTTP delivery while keeping event entry points separate from external processing targets.

- `IntegrationSource` represents where an event enters the platform.
- `IntegrationDestination` represents where that event is processed.
- `EventProcessorResolver` chooses the processor for the destination type.
- `N8nEventProcessor` builds the outbound envelope and performs delivery through `HttpClientContract`.
- `CurlHttpClient` is the native cURL implementation for JSON `POST`.
- `EventDispatcher` reserves events and marks outcomes, but does not know n8n details.
- `RetryPolicy` decides whether transient failures become retry or final failure.
- The scheduler only runs eligible events and stale recovery.

Migration `006_create_integration_destinations.sql` already provides `integration_destinations.config JSONB`, so Sprint 6.6B does not add a migration. For `type = n8n`, `config` stores only non-sensitive transport settings:
...
`webhook_url` is required for n8n destinations. Production requires HTTPS. The URL validator rejects empty or invalid URLs, unsupported schemes, localhost, loopback, private RFC1918 IPv4 ranges, link-local addresses, `169.254.0.0/16`, `0.0.0.0`, `::1`, and URLs with userinfo such as `https://user:pass@example.com`. The protection is intentionally basic SSRF hardening and does not attempt DNS rebinding defense in this sprint.

`timeout_seconds` defaults to `10`, must be an integer, and must be between `1` and `30`.

Administrative destination routes are owner-only and protected by CSRF on mutations:
...
`integration_sources.destination_id` is nullable so existing sources continue working. A source may have no destination. When a destination is selected, the service validates that source and destination belong to the same tenant. Active destinations of the selected tenant are offered for normal linking; an existing inactive destination remains visible in source detail/edit so historical configuration is not hidden or deleted.

The destination form exposes `Webhook URL` and `Timeout`; it does not expose a generic JSON editor. Destination detail shows the code, name, tenant, type, status, timeout, and a masked endpoint such as `https://n8n.example.com/.../hpucca-events`. Query strings are not shown.

The real dispatch path is:

```text
event -> source -> destination -> resolver -> processor
```

For `type = n8n`, the resolver returns `N8nEventProcessor`. The processor sends this controlled envelope:
...
It does not send internal database IDs, API key hashes, destination config, sessions, stack traces, credentials, secrets, Authorization headers, bearer tokens, HMAC signatures, or response bodies. The outbound JSON payload limit is `256 KB`; oversized payloads fail permanently before HTTP.

HTTP `2xx` is success. Transport failures and HTTP `408`, `425`, `429`, `500`, `502`, `503`, and `504` are transient and are handed back to `RetryPolicy`. Missing destination, inactive destination, invalid config, rejected URL, oversized payload, HTTP `400`, `401`, `403`, `404`, `405`, `410`, other non-transient `4xx`, and unexpected statuses are permanent. Unknown exceptions are treated as permanent. The response body is not used to decide success and is not persisted.

The event detail page shows the destination code/name and processor type when available. It does not show webhook URLs, config JSON, response body, secrets, or credentials.

Real HTTP integration tests are opt-in only:

```bash
RUN_N8N_HTTP_INTEGRATION_TESTS=1 N8N_TEST_WEBHOOK_URL=https://example.test/webhook php tests/N8nHttpIntegrationTest.php
```

Without the flag and URL, the test skips explicitly.
